...
- (UIImageView *)portrait {
    if (!_portrait) {
        _portrait = [[UIImageView alloc] initWithFrame:CGRectMake(16, 16, 60, 60)];
...
- (UILabel *)displayName {
    if (!_displayName) {
        _displayName = [[UILabel alloc] initWithFrame:CGRectMake(16 + 60 + 20, 16, [UIScreen mainScreen].bounds.size.width - 64, 32)];
...
- (UILabel *)userName {
    if (!_userName) {
        _userName = [[UILabel alloc] initWithFrame:CGRectMake(16 + 60 + 20, 16 + 32 + 8, [UIScreen mainScreen].bounds.size.width - 128, 14)];
